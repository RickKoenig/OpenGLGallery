...
import static com.mperpetuo.openglgallery.enga.VECNuMath.cross3d;
import static com.mperpetuo.openglgallery.enga.VECNuMath.dot3d;

/**
 * Created by cyberrickers on 11/15/2016.
 */

public class CollUtil {
    private static final String TAG = "CollUtil";
    public static int MAXBOX2BOX = 72;
    public static int MAXBOX2PLANE = 12;

    static VEC[] pln0 = new VEC[3];
    static VEC[] pln1 = new VEC[3];

    static VEC[] intpnts = new VEC[8];
...
*/
    ...
    static boolean line2bplane(VEC v0i,VEC v1i,VEC v2i,VEC top,VEC bot,VEC intsect) {
        VEC uvec = new VEC(),vvec = new VEC();
        VEC nrm = new VEC();
        float topdn,botdn,pdn,pdi;
        float t,b0,b1;
        VEC np1 = new VEC(),np2 = new VEC();
        VEC v0 = new VEC(),v1 = new VEC(),v2 = new VEC();//,cm;
        v0.copy(v0i);
        v1.copy(v1i);
        v2.copy(v2i);
        uvec.x = v1.x - v0.x;
...
// solve for intersection of line with plane
        ...
        boolean nx = Float.isNaN(intsect.x);
        boolean ny = Float.isNaN(intsect.y);
        boolean nz = Float.isNaN(intsect.z);
        if (nx || ny || nz)
            Log.e(TAG,"intsect is a NaN");
// see if on plane
...
*/
    ...
    static boolean line2btricull(VEC v0, VEC v1, VEC v2, VEC top,VEC bot,VEC intsect) {
        ...
        VEC nrm = new VEC();
        float topdn,botdn,pdn;
        VEC v0mi = new VEC(),v1mi = new VEC(),v2mi = new VEC(),c0 = new VEC(),c1 = new VEC(),c2 = new VEC();
        float d0,d1,d2;
        float t;
        uvec.x = v1.x - v0.x;
        uvec.y = v1.y - v0.y;
...
// now check to see if intsect is on the triangle
    ...
    static boolean plane2plane(VEC[] pl0,VEC[] pl1,VEC res0,VEC res1) {
        ...
        VEC maxpnts = new VEC(),minpnts = new VEC();
        //VEC intpnts[8];
        VEC pl03 = new VEC(),pl13 = new VEC();
        VEC maxpl0 = new VEC(),minpl0 = new VEC(),maxpl1 = new VEC(),minpl1 = new VEC();
        VEC delu = new VEC(),delv = new VEC(),nrm0 = new VEC(),nrm1 = new VEC(),crs = new VEC(),t = new VEC(),del = new VEC();
        ...
        if (npnts == 2) {
            ...
            minpnts.copy(intpnts[0]);
            maxpnts.copy(intpnts[0]);
            for (i=1;i<npnts;i++) {
            ...
            if (bmin<0 || bmax<0) {
                Utils.alert("this error just shouldn't happen");
                //Log.e(TAG,"this error just shouldn't happen");
            } else {
                res0.copy(intpnts[bmin]);
                res1.copy(intpnts[bmax]);
            }
        }
...
//	VEC b0max,b1max,b0min,b1min;
	...
	for (j=0;j<6;j++) {
        for (k=0;k<3;k++)
            pln1[k].copy(b1[pntplnidx[j][k]]);
        for (i=0;i<6;i++) {
            for (k=0;k<3;k++)
                pln0[k].copy(b0[pntplnidx[i][k]]);
            if (plane2plane(pln0,pln1,res[npnts],res[npnts + 1]))
                npnts += 2;
        }
    }
        if (npnts < 4 && npnts > 0) {
