...
void main(void) {
	highp float a = texture2D(uSampler0,vTextureCoord,-1.0).a;
	if (a < alphacutoff)
		discard;
#ifdef INVERT
    highp float epsilon = .0001;
	highp float w = 1.0 - gl_FragCoord.z - epsilon;
#else
	highp float w = gl_FragCoord.z;
#endif
//	w = .5;
...
void main(void) {
	//highp float a = texture2D(uSampler,vTextureCoord,-1.0).a;
	//if (a < alphacutoff)
	//	discard;
	highp float w = gl_FragCoord.z;
//	w = .5;
...
// Just copy3 the texture coordinate through
